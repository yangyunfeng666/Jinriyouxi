package com.jinriyouxi.gateway.filter;
...
public class TokenFilter implements GlobalFilter {
  ...
  public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
    ...
    if (request.getURI().toString().indexOf("author") > 0) {
      // 直接跳出 进入下一个认证chain
...
//    String token = exchange.getRequest().getQueryParams().getFirst("token");
    ...
    if (tokenHeader == null || !tokenHeader.startsWith("Bearer ")) {
      return noAuthorizationResponse(response);
    }    // 获取请求地址
  ...
  Mono<Void> noAuthorizationResponse(ServerHttpResponse response) {
    // 响应消息内容对象
    ResultJson resultJson = new ResultJson();
    resultJson.setCode(ResultCode.UNAUTHORIZED);
    resultJson.setMsg(ResultCode.UNAUTHORIZED.getMsg());
    resultJson.setData(null);
    // 转换响应消息内容对象为字节
    byte[] bits = resultJson.toString().getBytes(StandardCharsets.UTF_8);
